...
@Component({
    ...
    template: `
        ...
        <mat-dialog-actions>
            <button type="button" mat-button (click)="_cancel()">Anuluj</button>
            <button type="button" mat-button (click)="_save()">Zapisz</button>
        </mat-dialog-actions>
    `,
    styles: [`
        * {
...
export class SmdDataTableRowComponent {
    ...
    constructor(@Inject(forwardRef(() => SmdDataTable)) public _parent: SmdDataTable, private dialog: MatDialog, private viewContainerRef: ViewContainerRef) {
    }
    ...
    template: `
        <div>
            ...
            <span *ngIf="_hasRowsSelected()">
               Wybrano {{_selectedRowsLength()}} {{_selectedRowsLength() == 1 ? 'pozycję' : 'pozycje'}}
            </span>
        </div>
        <span>
            <div>
...
export class SmdDatatableHeader implements AfterContentInit, OnDestroy {
    ...
    public _hasRowsSelected(): boolean {
        return this._parent.selectedRows().length > 0;
    }

    public _selectedRowsLength(): number {
...
export class SmdDataTable implements DoCheck, AfterContentInit, OnDestroy {
    ...
    public rows: SmdDataRowModel[] = [];
    public visibleRows: SmdDataRowModel[] = [];
    private differ: any;
