...
export class NbTokenService {
  ...
  protected defaultConfig: any = {
    token: {
      ...
      getter: (): Observable<NbAuthSimpleToken> => {
        const tokenValue =this.backendService.getAccessTokenId(); 
  ...
  constructor(@Inject(NB_AUTH_OPTIONS_TOKEN) protected options: any,
              private backendService: LoopBackAuth,
  ...
  getConfigValue(key: string): any {
    let pom =getDeepFromObject(this.config, key, null);
    return pom;
  }
  ...
  get(): Observable<NbAuthSimpleToken> {
    let pom= this.getConfigValue('token.getter')();
    return pom;
  }
  ...
  tokenChange(): Observable<NbAuthSimpleToken> {
    return this.token$.asObservable();// publish().refCount();
  }
  ...
  public resetToken(token:string){
    ...
    this.token$.next(this.tokenWrapper);
  }
}
