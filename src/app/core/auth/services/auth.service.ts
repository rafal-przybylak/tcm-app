...
export class NbAuthService {
  ...
  public _userRoles:BehaviorSubject<string[]> = new BehaviorSubject<string[]>([]);
  ...
  public isAdministrator() {
    let adminRoles = CoreConfig.getAdminRoles();
    let isAdmin = false;
    this.getRole().subscribe(data => {
      if (data) {
        adminRoles.forEach(role => {
          isAdmin = isAdmin || data.includes(role);
        })
  ...
  getRole(): Observable<string[]> {
    return this.getProvider("email").getUserRole()
      .map((result: NbAuthResult) => {
        if (result.isSuccess()) {
          return result.getResponse() as string[];
        }
      }
      );
  }
